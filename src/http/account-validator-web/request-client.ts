 ...
 * library.
 */
export default class RequestClient2 extends AbstractClient {
    ...
    public async httpGetBinary (url: string, headers?: Headers): Promise<HttpResponse> {
        return this.requestBinary({ method: "GET", url, headers });
    }

    private async requestBinary (additionalOptions: AdditionalOptions): Promise<HttpResponse> {
        try {
            const options: AxiosRequestConfig = {
                method: additionalOptions.method,
                headers: {
